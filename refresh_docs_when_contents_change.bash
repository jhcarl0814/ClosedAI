...
while true
do
	time_of_last_access="$(find './contents' -printf '%p\n%t\n\n')";

	if [[ "$time_of_last_access" != "$time_of_last_access_last" ]]
	then
		printf '%s (%s)\n' "$(date +'%Y-%m-%d %H:%M:%S%:z')" "$(date --utc +'%Y-%m-%d %H:%M:%S%:z')";
		
		# awk_expand_commands './contents/git/git.template.html' | tee './docs/git/git.html' | awk '{ printf("| %s%s", $0, RT); }; END { if (RT == "" && $0 == "") printf("| \n"); else { if (RT == "") printf("\n"); else printf("| \n"); } }';
		awk_expand_commands './contents/git/git.template.html' >'./docs/git/git.html'
		printf '\n';
